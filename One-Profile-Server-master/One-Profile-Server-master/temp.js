...
app.use(
  cors({ origin: "https://one-profile-client.vercel.app", credentials: true })
);
...
app.post("/", verifyToken, async (req, res) => {
  ...
  res
    .status(201)
    .send({ message: "Post created successfully", newPost: result });
});

app.post("/like", verifyToken, async (req, res) => {
  const { postId } = req.body;
  const { _id } = req._id;
  if (!postId) res.status(500).send({ message: "Post id is required" });
...
async function getCFRating(userHandle) {
  ...
  if (result.status === "OK") return result.result[0].rating;
  else {
    console.log(result);
    return 0;
  }
...
async function getCC(username) {
  try {
    ...
    const globalRank = $(".rating-ranks > .inline-list > li > a")
      .children()
      .first()
      .text();
    const countryRank = $(".rating-ranks > .inline-list")
      .children()
      .last()
      .children()
      .first()
...
async function getLC(username) {
  try {
    ...
    const html = await axios.get(url);
    const $ = await cheerio.load(html.data);
    const details = $(".rating-contest-graph")
      .prev()
      .first()
      .first()
      .text()
...
async function updateRating() {
  ...
  let userWithRating = await Promise.all(
  ...
  try {
    ...
    let userWithRating = await Promise.all(
      userDb.map(async (user) => {
        let val = Math.max(
          user.cfrating,
          Number(user.ccrating) - 612,
          Number(user.atrating) + 222,
          Number(user.sprating) - 317
        );
        if (val <= 300) val = 0;
        return {
          ...user._doc,
...
// this is file code of one profile
